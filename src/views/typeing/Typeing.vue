...
import { h, nextTick, ref, toRefs, watch } from "vue";
...
const useTypeEffect = (
  ...
  const contentScroll = () => {
    ...
    const scrollDistance =
      activeEleTop - containerHeight / 2 + childElementHeight / 2;
  ...
  const changeHanziBgColor = (key) => {
    ...
    if (!nextPinyin && key !== "Backspace") {
      ...
      if (CorrectPinyinInput) {
        ...
        const hanzi = new Audio(
          `https://api.oick.cn/txt/apiz.php?text=${char}&spd=2`
        );
        hanzi.currentTime = 0;
        hanzi.play();
      }
  ...
  // 查找单词索引 ///////////////////////////////////////////////////////
  const findFirstNon = (position, arr, direction = 0) => {
    // 判断搜索方向
    const isForward = direction === 0;
    // 准备处理字符
    if (!isForward) {
      arr = arr.slice(0, position).reverse();
    } else {
      arr = arr.slice(position, arr.length);
    }
    // 获取位置
    for (let i = 0; i < arr.length; i++) {
      ...
      if (!/^[a-zA-Z']$/.test(char)) {
        // 根据搜索方向返回位置
        return isForward ? i + position : (i - arr.length) * -1;
      }
    }
  ...
  // 发音 ///////////////////////////////////////////////////
  const wordPlay = (word) => {
    let msgVoice = new SpeechSynthesisUtterance(word);
    msgVoice.voice = speechSynthesis.getVoices().filter(function (voice) {
  ...
  const keydown = (e) => {
    ...
    if (key === "Backspace") {
      ...
      //////////////////////
      // 获取下一个单词
      let headIndex = findFirstNon(index.value, pinyinArr, 1);
      let tailIndex = findFirstNon(index.value, pinyinArr, 0);
      //
      let foundWord = "";
      if (tailIndex != -1) {
        if (headIndex == -1) {
          foundWord = pinyinArr.slice(0, tailIndex).join("");
        } else {
          foundWord = pinyinArr.slice(headIndex, tailIndex).join("");
        }
      }
      // 播放控制，只播放一次，
      if (!isPlay && playWord != foundWord) {
        wordPlay(foundWord);
        playWord = foundWord;
      } else {
        wordPlayConcel();
...
export default {
  ...
  setup() {
    ...
    const getNewContent = (newId) => {
      ...
      if (newId > Object.keys(ContentInfo.value).length)
        return router.replace({ name: "Typeing", params: { id: 1 } });

      if (language.value === "cn") {
        const { newContent: a, pinyinArr: b } = getPinyin(
          ContentInfo.value[newId]
        );
        newContent.value = a;
        pinyinArr.value = b;
      } else if (language.value === "en") {
